  ...
  return (
    <motion.main className="max-w-7xl mx-auto p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: "Users", count: stats.userCount, color: "bg-blue-500", icon: User, path: `/admin/${userId}/users` },
          { label: "Recipes", count: stats.recipeCount, color: "bg-green-500", icon: RestaurantMenu, path: `/admin/${userId}/recipes` },
          { label: "Events", count: stats.eventCount, color: "bg-purple-500", icon: Event, path: `/admin/${userId}/events` },
          { label: "Categories", count: stats.categoryCount, color: "bg-orange-500", icon: CategoryIcon, path: `/admin/${userId}/categories` },
        ].map((item) => (
      ...
      <section className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-lg">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href={`/admin/${userId}/categories`} className="text-blue-600 flex items-center gap-1">View All <ArrowRight className="w-4 h-4" /></Link>
        </div>
      ...
      <section className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-lg">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href={`/admin/${userId}/occasions`} className="text-blue-600 flex items-center gap-1">View All <ArrowRight className="w-4 h-4" /></Link>
        </div>
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          {occasions.map(occ => (
