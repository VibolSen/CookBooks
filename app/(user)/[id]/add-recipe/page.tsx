...
import {
  ChefHat, FileText, ImageIcon, Plus, Save, AlertCircle, CheckCircle, ArrowLeft, Camera, List, BookOpen
} from "lucide-react";
...
export default function AddRecipe() {
  ...
  const [note] = useState("");
  const [categoryOccasion, setCategoryOccasion] = useState<{ categoryId: string | null; occasionId: string | null; categoryName?: string; occasionName?: string }>({
    categoryId: null,
    occasionId: null,
  });
  ...
  const actuallySubmit = useCallback(async () => {
    ...
    try {
      ...
      formData.append("categoryId", categoryOccasion.categoryId!);
      formData.append("occasionId", categoryOccasion.occasionId!);
  ...
  useEffect(() => {
    if (categoryOccasion.categoryId && categoryOccasion.occasionId && !isSubmitting && !showSuccessModal) {
      actuallySubmit();
    }
            ...
            <AnimatePresence mode="wait">
              {activeSection === "basics" && (
                <motion.div key="basics" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-lg font-medium mb-2">Recipe Name *</label>
                    <input value={recipeName} onChange={e => setRecipeName(e.target.value)} required className="w-full p-4 rounded-xl border dark:bg-gray-700" placeholder="Yummy Pasta" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-lg font-medium mb-2">Prep Time (min) *</label>
                      <input type="number" value={prepTime} onChange={e => setPrepTime(e.target.value)} required className="w-full p-4 rounded-xl border dark:bg-gray-700" />
                    </div>
                    <div>
                      <label className="block text-lg font-medium mb-2">Cook Time (min) *</label>
                      <input type="number" value={cookTime} onChange={e => setCookTime(e.target.value)} required className="w-full p-4 rounded-xl border dark:bg-gray-700" />
                    </div>
                  </div>
              ...
              {activeSection === "details" && (
                <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-lg font-medium mb-2">Short Overview *</label>
                    <textarea value={overview} onChange={e => setOverview(e.target.value)} required className="w-full p-4 rounded-xl border dark:bg-gray-700 h-32" />
                  </div>
                  <div>
                    <label className="block text-lg font-medium mb-2">Full Description *</label>
                    <textarea value={description} onChange={e => setDescription(e.target.value)} required className="w-full p-4 rounded-xl border dark:bg-gray-700 h-48" />
                  </div>
                  <div className="flex justify-between">
                    <button type="button" onClick={() => setActiveSection("basics")} className="px-6 py-3 bg-gray-200 dark:bg-gray-700 rounded-xl">Back</button>
                    <button type="button" onClick={() => setActiveSection("ingredients")} className="px-6 py-3 bg-orange-500 text-white rounded-xl">Next</button>
                  </div>
                </motion.div>
              )}
              ...
              {activeSection === "ingredients" && (
                <motion.div key="ingredients" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div className="flex justify-between">
                    <button type="button" onClick={() => setActiveSection("details")} className="px-6 py-3 bg-gray-200 dark:bg-gray-700 rounded-xl">Back</button>
                    <button type="button" onClick={() => setActiveSection("instructions")} className="px-6 py-3 bg-orange-500 text-white rounded-xl">Next</button>
                  </div>
                </motion.div>
              )}
              ...
              {activeSection === "instructions" && (
                <motion.div key="instructions" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div className="flex justify-between">
                    <button type="button" onClick={() => setActiveSection("ingredients")} className="px-6 py-3 bg-gray-200 dark:bg-gray-700 rounded-xl">Back</button>
                    <button type="button" onClick={() => setActiveSection("images")} className="px-6 py-3 bg-orange-500 text-white rounded-xl">Next</button>
                  </div>
                </motion.div>
              )}
              ...
              {activeSection === "images" && (
                <motion.div key="images" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <button type="button" onClick={addImageInput} className="aspect-square flex items-center justify-center border-2 border-dashed rounded-xl"><Plus /></button>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <button type="submit" disabled={isSubmitting} className="px-8 py-3 bg-orange-500 text-white rounded-xl flex items-center gap-2">
                      {isSubmitting ? "Saving..." : <><Save size={18} /> Finish</>}
                    </button>
                  </div>
                </motion.div>
              )}
