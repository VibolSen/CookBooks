...
import { updateOccasion } from "@/app/actions/occasionActions";
...
const EditOccasionModal: React.FC<EditOccasionModalProps> = ({
  ...
  useEffect(() => {
    if (occasion) {
      ...
      setCategoryId(occasion.categoryId);
    }
  }, [occasion]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("id", occasion.id);
    ...
    <motion.div className="fixed inset-0 z-50 flex justify-center items-center bg-black/50">
      <motion.div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl max-w-md w-full">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold">Edit Occasion</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            <button type="submit" disabled={loading} className="px-4 py-2 bg-amber-600 text-white rounded disabled:opacity-50">
