...
export default function EventsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "live": return <div className="bg-gradient-to-r from-red-500 to-pink-500 text-white px-3 py-1 rounded-full text-xs font-bold animate-pulse">🔴 Ongoing</div>;
      case "upcoming": return <div className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-3 py-1 rounded-full text-xs font-bold">✨ Upcoming</div>;
      case "past": return <div className="bg-gradient-to-r from-gray-400 to-gray-600 text-white px-3 py-1 rounded-full text-xs font-bold">📅 Past</div>;
      default: return null;
    }
  };
        ...
        >
          ...
          <div className="p-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center text-sm text-gray-600"><Calendar className="h-4 w-4 text-purple-500 mr-2" /><span>Starts: {formatDate(event.startDate)}</span></div>
              {event.endDate && <div className="flex items-center text-sm text-gray-600"><Clock className="h-4 w-4 text-blue-500 mr-2" /><span>Ends: {formatDate(event.endDate)}</span></div>}
            </div>
          </div>
        </motion.div>
      </Link>
  ...
  const renderSection = (title: string, evts: any[], emoji: string) => {
    ...
    return (
      <div className="mb-12">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-6">{emoji} {title} ({evts.length})</h2>
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <main className="container mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-extrabold bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent mb-4">Amazing Events</h1>
          <p className="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">Join our incredible community events and create unforgettable memories! 🎉✨</p>
          <div className="mt-6 flex flex-wrap justify-center gap-4">
            <div className="text-sm bg-white/50 dark:bg-gray-800/50 px-4 py-2 rounded-full">📍 {events.length} total events</div>
            <div className="text-sm bg-green-100/50 dark:bg-green-900/20 px-4 py-2 rounded-full">🔴 {ongoingEvent.length} ongoing</div>
            <div className="text-sm bg-blue-100/50 dark:bg-blue-900/20 px-4 py-2 rounded-full">✨ {upcomingEvents.length} upcoming</div>
            <div className="text-sm bg-gray-100/50 dark:bg-gray-700/50 px-4 py-2 rounded-full">📅 {pastEvents.length} past</div>
          </div>
        </div>
